...
public class ConsumerContractTests {
	...
	public void givenAlligatorMaryExists_WhenAlligatorMaryIsRequested_ThenResponseIsOk() {
		...
		Assert.assertEquals("Mary", JsonPath.read(response.getBody(), "$.name"));
	}
	...
	@Pact(provider = PROVIDER_NAME, consumer = CONSUMER_NAME)
	public RequestResponsePact getAlligatorMaryError(PactDslWithProvider builder) {
		...
		RequestResponsePact responsePact = builder
			...
			.body(new PactDslJsonBody().stringValue("error", "Argh!"))
			.toPact();
	...
	@Test
	@PactVerification(value = PROVIDER_NAME, fragment = "getAlligatorMaryError")
	public void givenAnErrorOccurs_WhenAlligatorMaryIsRequested_ThenResponseIsServerError() {
		...
		Assert.assertEquals("Argh!", JsonPath.read(response.getBody(), "$.error"));
	}
}
